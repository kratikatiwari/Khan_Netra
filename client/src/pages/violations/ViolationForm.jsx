...
export default function ViolationForm({ violation, mines = [], onSave, onCancel }) {
  ...
  const onSubmit = async (data) => {
    try {
      if (violation?.id) {
        await violationsApi.update(violation.id, data);
        toast.success('Violation updated');
      } else {
        await violationsApi.create(data);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="form-group">
          ...
          <input {...register('category', { required: true })} className="input" placeholder="Ventilation, Fire Safety, Water Pollution..." />
        </div>
        <div className="form-group">
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <button type="submit" disabled={isSubmitting} className="btn-primary">
          {isSubmitting ? 'Saving...' : violation ? 'Update Violation' : 'Report Violation'}
        </button>
      </div>
    </form>
  );
